...
# COLLECT_GCC_OPTIONS='-mthumb' '-mfloat-abi=hard' '-mcpu=cortex-m85+nopacbti' '-O2' '-fmessage-length=0' 
# '-fsigned-char' '-ffunction-sections' '-fdata-sections' '-fno-strict-aliasing' '-Wunused' '-Wuninitialized' 
# '-Wall' '-Wextra' '-Wmissing-declarations' '-Wconversion' '-Wpointer-arith' '-Wshadow' '-Wlogical-op' 
# '-Waggregate-return' '-Wfloat-equal' '-g' '-T' 'fsp.ld' 
# '-LC:\Users\RTT\e2_studio\workspace\ra8p1/script' '-specs=nano.specs' 
# '-v' '-o' 'ra8p1_m85.elf' '-mlibarch=armv8.1-m.main+fp.dp+mve.fp' 
# '-march=armv8.1-m.main+fp.dp+mve.fp' '-dumpdir' 'ra8p1_m85.elf.'
...
if PLATFORM == 'gcc':
    ...
    DEVICE = ' -mcpu=cortex-m85+nopacbti -march=armv8.1-m.main+fp.dp+mve.fp -mlibarch=armv8.1-m.main+fp.dp+mve.fp -mthumb -mfpu=fpv5-sp-d16 -mfloat-abi=hard -ffunction-sections -fdata-sections'
    CFLAGS = DEVICE + ' -Dgcc'
